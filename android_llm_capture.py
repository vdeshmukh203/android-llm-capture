...
class CapturedCall:
    ...
    def prompt_tokens_estimate(self) -> int:
        """Rough word-to-token estimate (~4/3 words per token) for the request."""
        ...
        for msg in messages:
            ...
            if isinstance(content, str):
                ...
                # Multi-modal: content is a list of typed parts
                for part in content:
                    if isinstance(part, dict) and part.get("type") == "text":
                        total_words += len(part.get("text", "").split())
        return max(1, total_words * 4 // 3)
...
def parse_logcat_line(line: str, state: Dict) -> Optional[CapturedCall]:
    ...
    *state* is mutated to accumulate partial call info across calls.
    Returns a :class:`CapturedCall` when a complete request/response pair is
    detected; returns ``None`` otherwise.
    """
    line = line.strip()
    if not line:
        return None
    ...
    # Body accumulation — only consume recognised JSON lines; let others fall through
    # so that the response-detection code below can match them.
    if state.get("phase") == "request" and state.get("url"):
        m_body = _OKHTTP_BODY.search(line)
        if m_body:
            state["body_lines"].append(m_body.group("body"))
            return None
        # Non-body lines (headers, end-marker "-->" …) fall through to response check.

    # OkHttp response start
    ...
    # Response body — first JSON-looking line completes the call
...
class CaptureSession:
    """Live capture session that wraps an ``adb logcat`` subprocess."""
    ...
    def stop(self) -> None:
        """Terminate the running ``adb logcat`` subprocess."""
        if self._proc:
    ...
    def replay(self, call: CapturedCall, api_key: str) -> Dict[str, Any]:
        """Replay a captured call against the live API using *api_key*."""
...
def list_devices() -> List[str]:
    """Return serial strings for connected ADB devices (no ``adb offline`` entries)."""
    try:
...
def list_packages(device: Optional[str] = None) -> List[str]:
    """Return package names installed on the connected Android device."""
    cmd = ["adb"]
    if device:
        cmd += ["-s", device]
...
def _load_calls_from_jsonl(path: Path) -> List[CapturedCall]:
    """Deserialise a JSONL captures file into :class:`CapturedCall` objects."""
    calls: List[CapturedCall] = []
...
def main(argv=None) -> int:
    ...
    if args.command == "replay":
        calls = _load_calls_from_jsonl(Path(args.captures))
        if not calls:
    ...
    if args.command == "stats":
        ...
        with open(args.captures, encoding="utf-8") as fh:
            for line in fh:
                line = line.strip()
                if line:
                    calls_raw.append(json.loads(line))
        ...
        print(f"Total calls: {len(calls_raw)}")
        print("By provider:")
    ...
    if args.command == "gui":
        try:
            ...
            print(
                "GUI module not found. Ensure android_llm_capture_gui.py is installed.",
                file=sys.stderr,
            )
            return 1
        return 0
...
#: Alias used in paper and CHANGELOG.
